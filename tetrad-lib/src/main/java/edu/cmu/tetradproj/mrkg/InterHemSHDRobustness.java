 ...
 */
public class InterHemSHDRobustness {

    private String brain;
    private double discount;
    private int discountInt;

    public InterHemSHDRobustness(String brain_specs, double penalty_discount){
        brain = brain_specs;
        ...
        try {
            ...
            for (int j = 0; j < interpols.size(); j++){
                for (int k = 0; k < 100; k++) {
                    ...
                    // load parent graph
                    if (k < 9) {
                        graph = GraphUtils.loadGraphTxt(new File(directories.get(j) + "/" + interpols.get(j) + "_IMaGES_skew_graph_00" + Integer.toString(k + 1) + "_" + Integer.toString(discountInt) + ".txt"));
                    } else if (k < 99){
                        graph = GraphUtils.loadGraphTxt(new File(directories.get(j) + "/" + interpols.get(j) + "_IMaGES_skew_graph_0" + Integer.toString(k+1) + "_" + Integer.toString(discountInt) + ".txt"));
                    } else {
                        graph = GraphUtils.loadGraphTxt(new File(directories.get(j) + "/" + interpols.get(j) + "_IMaGES_skew_graph_" + Integer.toString(k+1) + "_" + Integer.toString(discountInt) + ".txt"));
                    }
            ...
            // shdSums of edge numbers, reinstantiated for each preprocessing method
            int graphSum;
            int homotopicSum;
            int heterotopicSum;
            ...
            // shdAverages the number of edges in each interhemispheric, homotopic, and heterotopic graph for each preprocessing method,
          ...
          //  System.out.println("All averaged up!");
            ...
            // lists of shdAverages and shdVariances of SHDs in each preprocessing method
            List<Double> shdAverages = new ArrayList<>();
            List<Double> shdVariances = new ArrayList<>();

            List<Double> homotopicSHDAverages = new ArrayList<>();
            List<Double> homotopicSHDVariances = new ArrayList<>();

            List<Double> heterotopicSHDAverages = new ArrayList<>();
            List<Double> heterotopicSHDVariances = new ArrayList<>();
            ...
            for (int i = 0; i < interpols.size(); i++) {
                ...
                for (int k = 0; k < interpolSHDs.size(); k++){
                    shdSum = shdSum + interpolSHDs.get(k);
                }
            ...
            FileWriter avgWriter = new FileWriter(new File("/Users/user/Documents/FMRI/Tetrad/" + brain + "/adjusted_SHD_Robust_interhemispheric_" + Integer.toString(discountInt) + ".txt"));
            BufferedWriter bavgWriter = new BufferedWriter(avgWriter);

            for (int i = 0; i < interpols.size(); i++){

                bavgWriter.write(interpols.get(i) + " SHD Average: " + Double.toString(shdAverages.get(i)) + "\n");
                bavgWriter.write(interpols.get(i) + " SHD Variance: " + Double.toString(shdVariances.get(i)) + "\n\n");
            }
            ...
            for (int i = 0; i < interpols.size(); i++) {
                ...
                homotopicSHDAverages.add(shdSum/4950.0);

                shdSum = 0;
                interpolSHDs = homotopicSHDLists.get(i);
                for (int j = 0; j < interpolSHDs.size(); j++){

                    shdDiff = Math.pow((interpolSHDs.get(j) - homotopicSHDAverages.get(i)), 2.0);
                    shdSum = shdSum + shdDiff;
                }
                homotopicSHDVariances.add(shdSum / 4950.0);

            }

            FileWriter homAvgWriter = new FileWriter(new File("/Users/user/Documents/FMRI/Tetrad/" + brain + "/adjusted_SHD_Robust_homotopic_" + Integer.toString(discountInt) + ".txt"));
            BufferedWriter bhomAvgWriter = new BufferedWriter(homAvgWriter);

            for (int i = 0; i < interpols.size(); i++){

                bhomAvgWriter.write(interpols.get(i) + " SHD Average: " + Double.toString(homotopicSHDAverages.get(i)) + "\n");
                bhomAvgWriter.write(interpols.get(i) + " SHD Variance: " + Double.toString(homotopicSHDVariances.get(i)) + "\n\n");
            }
            ...
            for (int i = 0; i < interpols.size(); i++) {
                ...
                heterotopicSHDAverages.add(shdSum/4950.0);

                shdSum = 0;
                interpolSHDs = heterotopicSHDLists.get(i);
                for (int j = 0; j < interpolSHDs.size(); j++){

                    shdDiff = Math.pow((interpolSHDs.get(j) - heterotopicSHDAverages.get(i)), 2.0);
                    shdSum = shdSum + shdDiff;
                }
                heterotopicSHDVariances.add(shdSum / 4950.0);

            }

            FileWriter hetAvgWriter = new FileWriter(new File("/Users/user/Documents/FMRI/Tetrad/" + brain + "/adjusted_SHD_Robust_heterotopic_" + Integer.toString(discountInt) + ".txt"));
            BufferedWriter bhetAvgWriter = new BufferedWriter(hetAvgWriter);

            for (int i = 0; i < interpols.size(); i++){

                bhetAvgWriter.write(interpols.get(i) + " SHD Average: " + Double.toString(heterotopicSHDAverages.get(i)) + "\n");
                bhetAvgWriter.write(interpols.get(i) + " SHD Variance: " + Double.toString(heterotopicSHDVariances.get(i)) + "\n\n");
            }

            bhetAvgWriter.flush();
            bhetAvgWriter.close();

            // write shdAverages to file
         /*   FileWriter writer = new FileWriter(new File("/Users/user/Documents/FMRI/Tetrad/" + brain + "/interhemispheric_robust_skew_shdAverages_" + Integer.toString(discountInt) + ".txt"));
...
*/
    ...
    public static void main(String...args){
        ...
        InterHemSHDRobustness both2 = new InterHemSHDRobustness("USM_Both", 2.0);
        InterHemSHDRobustness both4 = new InterHemSHDRobustness("USM_Both", 4.0);
        InterHemSHDRobustness both6 = new InterHemSHDRobustness("USM_Both", 6.0);

        InterHemSHDRobustness aut_both2 = new InterHemSHDRobustness("USM_Aut_Both", 2.0);
        InterHemSHDRobustness aut_both4 = new InterHemSHDRobustness("USM_Aut_Both", 4.0);
        InterHemSHDRobustness aut_both6 = new InterHemSHDRobustness("USM_Aut_Both", 6.0);
